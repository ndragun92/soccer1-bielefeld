const project = {
  name: 'Soccer 1 - Bielefeld',
  description: 'Soccer 1 - Random Team Generator',
  url: 'https://ndragun92.github.io/soccer1-bielefeld/',
  // Nuxt Config
  lang: 'en',
  favicon: 'favicon.ico', // 228x228
  pwaIcon: 'https://ndragun92.github.io/soccer1-bielefeld/pwaIcon.png', // 512x512
  socialShareImage: 'favicon.ico',
  ogImage: 'https://ndragun92.github.io/soccer1-bielefeld/images/soccer1-logo.png',
  twitterImage: 'https://ndragun92.github.io/soccer1-bielefeld/images/soccer1-logo.png',
  themeColor: '#000000',
}

export { project }
